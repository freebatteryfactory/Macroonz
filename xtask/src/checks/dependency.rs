...
//! `cargo metadata` is asked directly. Its answer is a role-distinct live
//! observation taken beside the committed file projection; Cargo's executable,
//! environment, configuration, and cache do not become committed facts.
//!
...
mod tests {
    ...
    /// Aggregate reversal: a malformed committed manifest makes the derived
    /// census unreadable. It cannot silently contribute an empty edge set.
    #[test]
    fn an_unreadable_committed_manifest_refuses_the_dependency_census() -> Result<(), String> {
        let scratch = Scratch::named("dependency-unreadable-manifest")?;
        scratch.write("Cargo.toml", "[package\nname = \"broken\"\n")?;
        let snapshot = scratch.read()?;
        let found = check_no_core_tooling_edge(&snapshot);
        assert!(
            found.is_err_and(
                |refusal| refusal.contains("committed manifest dependency census")
                    && refusal.contains("Cargo.toml")
                    && refusal.contains("could not be read")
            ),
            "an unreadable manifest became an empty census contribution"
        );
        Ok(())
    }

    /// Aggregate reversal for the role-distinct live Cargo leg. The committed
    /// declaration is written at the innocent local key `helpers` and inherits
    /// both package identity and path from the workspace pool, so the local
    /// manifest census alone reports no offence. Cargo normalization resolves
    /// that inherited declaration to the forbidden services package.
    #[test]
    fn cargo_normalization_catches_an_inherited_renamed_edge() -> Result<(), String> {
        let scratch = Scratch::named("dependency-normalized-inheritance")?;
        scratch.write(
            "Cargo.toml",
            "[package]\nname = \"threadpak\"\nversion = \"0.0.0\"\nedition = \"2024\"\n\n\
        ...
        let census = snapshot
            ...
            .census()
            .required("the committed manifest dependency census")?;
        let declared_offences: Vec<_> = census
            .of("Cargo.toml")
            .into_iter()
            .filter_map(judge_core_declaration)
            .collect();
        assert!(
            declared_offences.is_empty(),
            "the hostile did not isolate Cargo normalization: {declared_offences:?}"
        );

        let found = check_no_core_tooling_edge(&snapshot);
        assert!(
            found.is_err_and(
                |refusal| refusal.contains("as cargo normalized the manifests")
                    && refusal.contains("threadpak-macroc")
            ),
            "the live Cargo observation did not catch the inherited renamed edge"
        );
        Ok(())
    }
    ...
    ///
    /// This reaches Cargo's live normalization over the actual workspace, while
    /// the aggregate hostile above proves that leg can independently activate.
    /// It also exercises refusal-on-unknown: a run that could not ask Cargo
    /// fails here rather than reporting an absence nobody established.
    #[test]
