...
///
/// One value rather than three loose readings, because the three are one
/// declaration's: a semantic name read off one capture beside a carrier anchor
/// read off another is a pair nothing keeps honest, and the seats below compare
/// whole readings for exactly that reason.
...
struct Readings {
    ...
    /// What its CARRIER stands on.
    carrier: ProjectionIdentity<CapturedDeclarationSubject>,
}
...
fn readings(source: &str) -> Option<Readings> {
    ...
    Some(Readings {
        ...
        carrier: surface.carrier_anchor(),
    })
}
...
fn grammar_cause(source: &str) -> Option<TrialDeclarationCause> {
    match refused(source)? {
        ...
        | SurfaceCaptureRefusal::Declaration(_) => None,
    }
}
...
fn a_trial_edit_moves_the_carrier_and_leaves_the_implementation() {
    ...
    assert_ne!(one.map(|read| read.carrier), other.map(|read| read.carrier));
}

/// A declaration that states no trials anchors its carrier on the declaration
/// itself, and one that states them does not.
///
/// The two postures never read alike, which is what keeps "this declaration
/// declared no rows" from being a stand-in for "this declaration declared some
/// and the reader dropped them".
#[test]
fn a_declaration_with_no_trials_anchors_its_carrier_on_itself() {
    let bare = captured_text(&declaration("")).ok();
    assert!(bare.as_ref().is_some_and(|(_, surface)| matches!(
        surface.trials(),
        TrialDeclarationPosture::NotDeclared
    )));
    assert!(readings(&declaration("")).is_some_and(|read| read.semantic == read.carrier));
...
fn three_readings_of_one_declaration_reach_three_names() {
    ...
    assert!(read.is_some_and(|read| {
        read.semantic != read.documentation
            && read.semantic != read.carrier
            && read.documentation != read.carrier
    }));
}
