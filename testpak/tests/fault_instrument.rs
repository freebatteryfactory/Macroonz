...
const CONSUMER: &str = "testpak.fault.consumer";
...
fn schedule_name(
    stem: &'static str,
) -> Result<threadpak_testpak::descriptor::NamespacedName, NameRefusal> {
    threadpak_testpak::descriptor::NamespacedName::named(CONSUMER, stem)
}
...
fn apply_command(
    state: &WriteState,
    injected: &threadpak_testpak::fault::InjectedCommand<
        WriteCommand,
        WriteFault,
        WritePostcondition,
    >,
...
fn contract(
    ...
    TransitionContract<
        WriteState,
        threadpak_testpak::fault::InjectedCommand<WriteCommand, WriteFault, WritePostcondition>,
    >,
    ContractRefusal,
> {
