...
//! is that supply's own length. Extending the supply therefore fails this file
//! loudly — the roster stops spending it — rather than letting it keep passing
...
fn a_stamped_roster_declares_its_own_ceiling() {
    assert_eq!(
        SpentSupply::ALL.len(),
        threadpak::CLOSED_REGISTER_ROW_CEILING
    );
