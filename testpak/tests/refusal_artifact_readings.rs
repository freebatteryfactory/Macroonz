...
use threadpak::refusal::{CauseOrderDeclaration, DeclaredCauseOrder, FamilyShape, RefusalFamily};
...
const DECLARED_SHAPE_PATH: &str = "::threadpak::refusal::FamilyShape::SingleCause";

/// The structural path for the body shape the damaged artifact states.
const DAMAGED_SHAPE_PATH: &str = "::threadpak::refusal::FamilyShape::IssueCollection";
...
mod lawful_artifact {
    ...
    #[expect(
        dead_code,
        reason = "the enum is the compiled artifact's type seat; this lane reads the generated trait constants and does not manufacture runtime variants"
    )]
...
mod shape_altered_artifact {
    ...
    #[expect(
        dead_code,
        reason = "the enum is the damaged compiled artifact's type seat; this lane reads the generated trait constants and does not manufacture runtime variants"
    )]
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    pub(crate) enum DemoFamily {
        /// A cause outside the machine's roster.
...
fn structural_cause(local: &str, spelling: &str) -> ConstantReading {
    structural_call(
        "::threadpak::refusal::DeclaredCause::declared",
        vec![
            structural_call(
                "::threadpak::refusal::CauseId::declared",
                vec![
                    structural_call(
                        "::threadpak::refusal::RefusalFamilyId::declared",
                        vec![ConstantReading::Text(DECLARED_FAMILY.to_owned())],
                    ),
                    structural_call(
                        "::threadpak::refusal::LocalCauseKey::declared",
...
fn structural_verdict(rendered: &str, shape_path: &str) -> StructuralVerdict {
    ...
    let order_members = [DeclaredMember {
        ...
        reading: structural_call(
            "::threadpak::refusal::DeclaredCauseOrder::declared",
    ...
    let implementations = [
        DeclaredImplementation {
            target: "DemoFamily",
            trait_path: "::threadpak::refusal::RefusalFamily",
            postures: &[],
            attributes: &[],
        ...
        DeclaredImplementation {
            target: "DemoFamily",
            trait_path: "::threadpak::refusal::CauseOrderDeclaration",
            postures: &[],
            attributes: &[],
...
fn compiled_observation<Family>() -> CompiledObservation
where
