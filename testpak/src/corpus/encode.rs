//! Canonical seed-pack body encoding and the one public writer.
...
/// # Errors
///
/// Refuses no seed, then the first exact duplicate seed. Every other field is already informed, and the writer derives rather than accepts the address.
pub fn pack(population: PopulationRef, seeds: Vec<SeedInput>) -> Result<SeedPack, SeedPackRefusal> {
    ...
    encoded.extend_from_slice(address.address().as_bytes());
    encoded.extend_from_slice(&body);
    SeedPack::assembled(population, address, seeds, encoded)
}

/// The complete address preimage of one seed pack.
#[must_use]
pub(crate) fn encode_body(population: PopulationRef, seeds: &[SeedInput]) -> Vec<u8> {
