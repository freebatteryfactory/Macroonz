...
The gate consumes two typed facts — the adapter qualification, and the compiled-pressure witness shown under it — and never a bare run stripped of the profile that says which tool, which version posture, which output, and which grammar produced the evidence. Neither fact exists over an adapter nobody has checked, so what reaches the gate is either evidence somebody vouched for or nothing at all.
