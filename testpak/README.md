...
testpak's production library is standalone: `arbitrary`, `blake3`, and no crate of this workspace. Its qualification surface depends inward on `threadpak`, `threadpak-macroc`, and `threadpak-macros`, and uses `syn` for structural challenges, all as dev-dependencies reached from `tests/`, where its own qualification runs.
