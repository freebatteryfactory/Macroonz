...
//! [`preemption`] explores instruction-level interleavings and the memory model through the pinned loom scheduler (off by default; the `preemption` feature opts in, and the workspace wall runs it every time).
