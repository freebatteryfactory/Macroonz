//! Constructors, readers, and invariant checks for deterministic network simulation.
...
impl Tick {
    ...
    #[must_use]
    pub(in crate::network) const fn next(self) -> Self {
...
impl<'campaign> NetworkSelection<'campaign> {
    ...
    /// Retain one already-informed schedule for transcript reproduction inside the network home.
    #[must_use]
    pub(in crate::network) const fn retained(schedule: &'campaign NetworkSchedule) -> Self {
        Self { schedule }
    }
}
...
impl<Payload> SimNet<Payload> {
    ...
    /// The topology retained for transcript projection inside the network home.
    #[must_use]
    pub(in crate::network) const fn retained_topology(&self) -> &Topology {
        &self.topology
    }

    /// The selected schedule retained for transcript projection inside the network home.
    #[must_use]
    pub(in crate::network) const fn retained_schedule(&self) -> &NetworkSchedule {
        &self.schedule
    }

    /// The successful actions retained for transcript projection inside the network home.
    #[must_use]
    pub(in crate::network) fn retained_actions(&self) -> &[Action<Payload>] {
        &self.actions
    }

    /// The deliveries retained for transcript projection inside the network home.
    #[must_use]
    pub(in crate::network) fn retained_history(&self) -> &[Delivery<Payload>] {
        &self.history
    }
}
...
impl<Payload> Delivery<Payload> {
    ...
    #[must_use]
    pub(in crate::network) const fn delivered(
        link: Link,
        ordinal: SendOrdinal,
        payload: Payload,
        sent_at: Tick,
