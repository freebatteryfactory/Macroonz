...
pub fn retain_capsule(
    ...
    encode_bytes(&key_bytes, &mut body);
    if let Some(input) = key.input() {
        body.push(1);
        let profile = input.profile();
        encode_bytes(profile.name().namespace().written().as_bytes(), &mut body);
        encode_bytes(profile.name().stem().written().as_bytes(), &mut body);
...
fn sum(parts: &[usize]) -> Result<usize, ArchiveRefusal> {
...
fn bounded(length: usize, limits: ArchiveLimits) -> Result<usize, ArchiveRefusal> {
...
fn encoded_size(capsule: &ReplayCapsule, limits: ArchiveLimits) -> Result<usize, ArchiveRefusal> {
    ...
    let total = sum(&[69, key_size, fingerprint_size, capsule_size, metadata])?;
