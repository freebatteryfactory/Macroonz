...
use macroonz::{Bounded, ConstLimit};
...
use threadpak_macroc::{
    AccountedExpansion, AssemblyIssue, ClosureIssue, ExplanationBindingRefusal, ExplanationSeat,
    HumanProjection, MacrocPhase, MutationDeclarationPosture, ObservedClassification,
    PlanDecisions, PlannedMembership, ProjectionClosure, ProjectionPlan, ProjectionPlanningIssue,
    RefusalCompileContext, RefusalDeriveCapture, RefusalFamilyExpansion,
    RefusalFamilyImplementationProjection, RefusalOwnerFacts, RelatedIdentity,
    RenderedImplementation, RenderedProjection, RenderedRole, RenderedUnit, ReproductionRoute,
    SoleRenderedUnit, TextCapture, TextCompileRefusal, TrialDeclarationPosture,
    compile_declaration, compile_refusal_text,
};
...
fn delivered_with_trials(claim: &str) -> Result<AccountedExpansion<RefusalFamilyExpansion>, ()> {
...
) -> ProjectionPlan<RefusalFamilyImplementationProjection> {
...
fn assert_mutation_clause_roster_refusals() {
    let unknown = r#"
        #[refusal(family = "testpak.demo", shape = single_cause, order(A = "a", B = "b"))]
        #[threadpak_mutations(
            support = demo_mutations,
...
fn assert_mutation_attribute_shape_refusals() {
...
fn assert_mutation_clause_value_refusals() {
    let cases = [
        (
        ...
        (
            mutation_source(
                &VALID_MUTATION_BODY.replace("map declared_order =", "map declared_order"),
            ),
        ...
        (
            mutation_source(&VALID_MUTATION_BODY.replace("[\"declared-order-permutation\"]", "[]")),
        ...
        (
            mutation_source(
                &VALID_MUTATION_BODY.replace("declared-order-permutation", "invented-operator"),
            ),
...
fn assert_mutation_cross_helper_refusals() {
    ...
    let owner_unmapped = VALID_MUTATION_BODY.replace(
        "    map declared_order = named(\"testpak\", \"demo-order\"),\n",
        "",
    );
    let unavailable_unmapped_fact = format!(
        "#[refusal(family = \"testpak.demo\", shape = inseparable_pair)] \
         #[threadpak_mutations({owner_unmapped})] enum DemoFamily {{ A, B }}"
    );
    assert!(mutation_refused_as(
        &unavailable_unmapped_fact,
        MutationDeclarationCause::OwnerFactNotAvailable,
        ObservedClassification::ContractDisagreement,
    ));
...
fn the_mutation_helper_consumes_every_owned_token_once() {
    assert_mutation_clause_roster_refusals();
    assert_mutation_attribute_shape_refusals();
    assert_mutation_clause_value_refusals();
    assert_mutation_cross_helper_refusals();
}
...
fn mutation_commitment_moves_only_its_member_and_delivery() -> Result<(), ()> {
    ...
    assert_eq!(
        first_projected.surface().identity(),
        second_projected.surface().identity()
    );
    assert_eq!(
    ...
    for role in [
        ...
        assert_eq!(
            first_unit.tree().canonical_bytes(),
            second_unit.tree().canonical_bytes()
        );
    }
    ...
    assert_ne!(
        first_mutation.semantic_key(),
        second_mutation.semantic_key()
    );
...
#[test]
fn trial_and_mutation_commitments_remain_independent_where_both_are_delivered() -> Result<(), ()> {
...
fn mutation_dependencies_refuse_crossed_delivery_joins() -> Result<(), ()> {
    ...
    let assembly_refusal = threadpak_macroc::derive_refusal::assembly(
        &second_draft,
    ...
    let second_plan =
        threadpak_macroc::derive_refusal::carrier_plan(&second_draft).map_err(|_| ())?;
    let composition_refusal = threadpak_macroc::derive_refusal::carrier_expansion(
        &second_draft,
        second_plan,
