//! The family-isolation lane: what a domain separates, and what a position separates, observed from outside the crate that decides both.
//!
//! A type cannot carry either of these.
//! That two preimages under different families reach unrelated addresses, and that one family at two positions reaches unrelated addresses, are facts about the derivation context this crate assembles — so they are observed here, over the public roads, with no private seat touched.
//!
//! # Reversals
//!
//! The separations are required to HOLD APART rather than merely to exist.
//! A derivation that ignored the family segment would still hand back thirty-two bytes and would still look like an answer; what it would not do is give two families two names for one preimage.
//! Each observation below is a pair that must disagree, and the spelling observation is what says why.
...
/// One family at two positions over one preimage reaches two unrelated addresses.
///
/// This is what a position buys, and it is the whole of what a move performs: a family that advances renames every address it derives, so a reader holding the earlier name can tell it apart from the later one.
/// A position that did not reach the context would be a number in a doc comment.
#[test]
...
///
/// The two separations compose rather than substitute: moving one family leaves every address under every other family with its name, which is the whole reason a position rides its own tag instead of standing over the crate.
#[test]
...
///
/// The order is the reason a position belongs to the family it is written beside.
/// A version segment ahead of the family would read as the stem's, and this is where that reading is ruled out by the string itself rather than by a paragraph about it.
#[test]
