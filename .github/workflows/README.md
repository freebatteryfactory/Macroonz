...
GitHub supplies the standard public Windows and macOS runners for independent provider and operating-system diversity without spending premium runner minutes.
...
## Compiler and generated-runtime observations
...
Its runtime matrix sibling builds the facade and harness instead, including the proc carrier transitively, and executes generated dispatch, relation-table and codec code.
Both subjects use the same bounded launch and report path, with separate runner jobs, artifact roots and sample populations.
...
The runtime subject distinguishes increasing call counts over fixed structures from increasing structure widths at fixed call counts.
Independent finite transition and policy expectations, arithmetic cycle/equality models and exact big-endian codec vectors precede runtime measurement.
Runtime timing includes input selection, generated execution, consumed results and batch recording; codec calls also include their vector allocation and drop.
