...
fn mapping<'trees>(
    ...
    let fact = MutationOwnerFact::of_spelling(spelling).ok_or_else(|| {
        grammar(
...
fn operator_family(
    ...
    GeneratedMutationFamily::of_slug(slug).ok_or_else(|| {
        grammar(
...
fn distinct(clauses: &[Clause<'_>]) -> Result<(), MutationDeclarationRefusal> {
    for (position, clause) in clauses.iter().enumerate() {
        let duplicate = clauses.iter().take(position).any(|earlier| match (earlier, clause) {
            (
                Clause::Assigned { key: left, .. },
                Clause::Assigned { key: right, .. },
            ) => left == right,
        ...
        if duplicate {
            let (cause, at) = match clause {
                ...
                Clause::Mapping { at, .. } => {
                    (MutationDeclarationCause::DuplicateOwnerFact, *at)
                }
...
fn named_assignment(
    ...
    let (value, site) = assigned(clauses, key)
        .ok_or_else(|| grammar(MutationDeclarationCause::NotCovered, at))?;
