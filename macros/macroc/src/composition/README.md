...
The one place that says which providers of descriptor material exist, so a
consumer that must see all of them at once has a single declared set to read.
...
carries none: it is a list that happens to be right when it is written, and
nothing refuses when it stops being right. The machine's format law bans
hand-maintained inventories for precisely that reason, and this root is not one —
every claim it makes is joined against derived facts.
...
and its shape is stated here so the obligation is not vague: a provider exists ↔
it appears exactly once in this root ↔ it has a disposition ↔ it has an
obligation. Omission fails, phantom fails, duplicate fails. This home owns the
duplicate end structurally — [`CompositionRoot::declared`] refuses one — and the
omission and phantom ends bind once the providers themselves exist.
