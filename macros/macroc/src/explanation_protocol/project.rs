...
pub(super) fn human_line(answer: &ExplanationAnswer) -> HumanProjection<HumanTextLimit> {
    match answer {
        ...
        ExplanationAnswer::GraphAndProfile { .. } => human_projection!(
            HumanTextLimit,
            "what the plan was decided against, and the projection profile at its version"
        ),
        ExplanationAnswer::SelectedWrappers { .. } => human_projection!(
            HumanTextLimit,
            "the decision trace that selected this output's wrapper components"
        ),
        ...
        ExplanationAnswer::CorrespondingRuntimeTraces { .. } => human_projection!(
            HumanTextLimit,
            "the runtime traces that correspond to this output"
        ),
