...
/// The machine's refusal home, as the emission names it.
pub const REFUSAL_HOME: &str = "refusal";

/// The machine's root shape calculus, as the emission names it.
pub const TYPES_HOME: &str = "types";
...
/// The family contract's selection-order seat.
pub const SELECTION_ORDER_SEAT: &str = "SELECTION_ORDER";
...
pub fn seat_module(form: SeatMintForm) -> Result<Vec<GeneratedToken>, StampRenderIssue> {
    ...
    seat.extend(machine_path(&[REFUSAL_HOME, ADMITTED_PREFIX]));
    ...
    body.push(GeneratedToken::word("impl"));
    body.extend(machine_path(&[REFUSAL_HOME, REFUSAL_FAMILY]));
...
fn mint_road() -> Result<Vec<GeneratedToken>, StampRenderIssue> {
    ...
    admitted.extend(machine_path(&[TYPES_HOME, POSITIVE_LIMIT]));
    ...
    examined.extend(machine_path(&[REFUSAL_HOME, STOP_BOUND]));
    ...
    built.extend(machine_path(&[REFUSAL_HOME, ADMITTED_PREFIX]));
...
fn issues_road() -> Result<Vec<GeneratedToken>, StampRenderIssue> {
    ...
    returned.extend(machine_path(&[TYPES_HOME, NON_EMPTY_BOUNDED]));
...
fn posture_road() -> Result<Vec<GeneratedToken>, StampRenderIssue> {
    reader_road(
        POSTURE_SENTENCE,
        POSTURE_ROAD,
        machine_path(&[REFUSAL_HOME, COMPLETION_POSTURE]),
        COMPLETION_ROAD,
    )
}
...
/// The family contract every coupled seat realizes: the shape it always takes,
/// and the empty selection order an independent-member family declares.
///
/// Both are the STAMP's and neither is the caller's. A coupled seat IS an issue
/// collection — that is what the private body's type means — and an issue
/// collection elects no primary issue, so there is no textual order for a caller
/// to state.
fn family_contract() -> Result<Vec<GeneratedToken>, StampRenderIssue> {
    let mut tokens = vec![
        GeneratedToken::word("const"),
        GeneratedToken::word(SHAPE_SEAT),
        GeneratedToken::alone(':'),
    ];
    tokens.extend(machine_path(&[REFUSAL_HOME, FAMILY_SHAPE]));
    tokens.push(GeneratedToken::alone('='));
    tokens.extend(machine_path(&[REFUSAL_HOME, FAMILY_SHAPE]));
    tokens.push(GeneratedToken::joint(':'));
    tokens.push(GeneratedToken::alone(':'));
    ...
    tokens.push(GeneratedToken::word("const"));
    tokens.push(GeneratedToken::word(SELECTION_ORDER_SEAT));
    tokens.push(GeneratedToken::alone(':'));
    tokens.push(GeneratedToken::alone('&'));
    tokens.push(GeneratedToken::joint('\''));
    tokens.push(GeneratedToken::word("static"));
    tokens.push(group(
        GeneratedDelimiter::Bracket,
        vec![
            GeneratedToken::alone('&'),
    ...
    tokens.push(GeneratedToken::alone('='));
    tokens.push(GeneratedToken::alone('&'));
    tokens.push(group(GeneratedDelimiter::Bracket, Vec::new())?);
    tokens.push(GeneratedToken::alone(';'));
    Ok(tokens)
}
