...
/// The independently reconstructed generated bytes for the exact-literal fixture.
fn exact_literal_receipt() -> Result<Vec<u8>, ()> {
    let mut bytes = vec![8, 0];
    framed(b"0xFFu8", &mut bytes)?;
    bytes.extend_from_slice(&[8, 1]);
    framed("é".as_bytes(), &mut bytes)?;
    bytes.extend_from_slice(&[8, 2, 0xff, 8, 3]);
    framed(b"ab", &mut bytes)?;
    Ok(bytes)
}
...
/// Hostile control: the expected generated bytes are reconstructed independently rather than read through the implementation encoder.
