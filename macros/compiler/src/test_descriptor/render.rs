...
//! invocation.
//! A generated row points at a check function the CONSUMPTION target owns, which arrives as an expression rather than as a rendered path.
//! The generated mutation module in the deferred seat spells the binding the DECLARATION stated.
//! The twin stays on the wall's roster because the BENCH crossing rides this same carrier and its rows do point at machine callables.
//!
...
use super::{
    ...
    ShellRenderIssue, SuiteGroup, SupportDelivery, TrialDelivery, TrialLensName, TrialTablePayload,
    WallName,
};
...
///
pub(crate) fn deferred_cargo(deferred: DeferredDelivery<'_>) -> Vec<GeneratedToken> {
    let cargo = match deferred {
        // An expansion that planned no member into this carrier splices no
        // tokens at all.
        DeferredDelivery::NothingDeferred => return Vec::new(),
        DeferredDelivery::Carried(carried) => carried,
    };
    cargo.tree().tokens().cloned().collect()
}
