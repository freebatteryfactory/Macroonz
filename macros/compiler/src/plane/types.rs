...
use macroonz::{Bounded, Limit, LimitAdmissionProfile};
...
macro_rules! limits {
    ( $( $(#[$note:meta])* $name:ident = $max:expr ),+ $(,)?) => {
        $(
            ...
            #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
            pub struct $name;
            impl ::macroonz::Limit for $name {
                type Authority = ::macroonz::DeclaredMagnitude;
            }
            impl ::macroonz::ConstLimit for $name {
                const MAX: usize = $max;
            }
            ...
            impl $name {
                ...
                ///
                /// [`ConstLimit::MAX`]: macroonz::ConstLimit::MAX
...
///
/// Documentation is captured and committed under its own family.
/// A `#[doc = "…"]` attribute is declaration material whose text is a second reading rather than the declaration's semantics, so it is omitted here and enters [`DECLARATION_DOCUMENTATION_IDENTITY_PROFILE`] instead.
/// A declaration whose prose changes keeps its semantic name and takes a new inspectable documentation name.
///
...
/// # Authority
///
/// **A second reading of one surface, never a second semantic authority.**
/// The rows are cut from the same token material the semantic commitment stands over, so nothing here is a fact the capture did not already read.
/// This profile names that retained inspectable reading and grants it no additional meaning.
///
/// # Bounds
///
...
/// - **2** — the refusal-family implementation projection's rendered-role roster gained the generated mutation module role, so plans of that kind derive different identities from the larger declared output set.
/// - **3** — the delivery shape changed: the generated mutation module moved from the declaration-site tree into the support carrier, and the closed output became partitioned rather than one joined tree.
///   This change reaches identity through a member's declared destination and the emission member of the closure's own preimage.
...
/// - **5** — mutation evaluation reads production from the owner's typed declaration, delegates no-mutation evaluation to production, and emits active adjacent permutations separately; generated support binds its public address to the cargo owner that declared it.
pub const MACROC_GENERATOR: GeneratorIdentity = GeneratorIdentity::declared(
    GeneratorProfileId::declared("threadpak-macroc"),
    GeneratorSchemaVersion::declared(5),
