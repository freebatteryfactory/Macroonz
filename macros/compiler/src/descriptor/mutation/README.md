...
Nothing here holds a roster of facts that exist or families that can be applied, and nothing here generates an alternative. A producer that computed the pressure a declaration is under would be a producer that knew what the declaration means — which is exactly the thing it does not know.
