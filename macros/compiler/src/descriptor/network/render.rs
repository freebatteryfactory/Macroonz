...
const FAULT_ARMS: [(&str, [&str; 3], &str); 4] = [
    ...
    (
        "Topology",
        [HARNESS, "network", "TopologyRefusal"],
        "The declared topology was refused by its own guard.",
    ),
    (
        "Span",
        [HARNESS, "network", "TickSpanRefusal"],
        "A declared delay span was refused by its own guard.",
    ),
    (
        "Schedule",
        [HARNESS, "network", "NetworkScheduleRefusal"],
...
/// One `::macroonz_harness::network::NodeRef::declared(::…::NamespacedName::named("<ns>", "<node>")?)` expression.
fn node_expr(namespace: &str, node: &str, into: &mut Vec<GeneratedToken>) -> Result<(), Overflow> {
    absolute_path(&[HARNESS, "network", "NodeRef", "declared"], into);
    let mut inner = Vec::new();
    absolute_path(
        &[HARNESS, "descriptor", "NamespacedName", "named"],
        &mut inner,
    );
...
/// One `::macroonz_harness::network::Link::between(<from>, <to>)` expression.
fn link_expr(
    namespace: &str,
    link: &LinkRow,
    into: &mut Vec<GeneratedToken>,
) -> Result<(), Overflow> {
    absolute_path(&[HARNESS, "network", "Link", "between"], into);
    let mut inner = Vec::new();
    node_expr(namespace, link.from(), &mut inner)?;
    inner.push(GeneratedToken::alone(','));
    node_expr(namespace, link.to(), &mut inner)?;
    into.push(GeneratedToken::group(
        GeneratedDelimiter::Parenthesis,
        inner,
    )?);
...
fn topology_fn(
    ...
    let mut inner = Vec::new();
    absolute_path(&[HARNESS, "network", "Topology", "declared"], &mut inner);
    let mut nodes = Vec::new();
    for node in declaration.nodes() {
        let mut expression = Vec::new();
        node_expr(declaration.namespace(), node, &mut expression)?;
        nodes.push(expression);
    }
    let mut links = Vec::new();
    for link in declaration.links() {
        let mut expression = Vec::new();
        link_expr(declaration.namespace(), link, &mut expression)?;
...
fn schedule_fn(
    ...
    let mut inner = Vec::new();
    absolute_path(
        &[HARNESS, "network", "NetworkSchedule", "declared"],
        &mut inner,
    );
    let mut seats = Vec::new();
    absolute_path(
    ...
    for discipline in schedule.disciplines() {
        let mut expression = Vec::new();
        discipline_expr(declaration.namespace(), discipline, &mut expression)?;
...
/// One `::macroonz_harness::network::LinkDiscipline::declared(<link>, <faults>)` expression.
fn discipline_expr(
    namespace: &str,
    discipline: &DisciplineRow,
    into: &mut Vec<GeneratedToken>,
) -> Result<(), Overflow> {
    absolute_path(&[HARNESS, "network", "LinkDiscipline", "declared"], into);
    let mut seats = Vec::new();
    link_expr(namespace, discipline.link(), &mut seats)?;
    seats.push(GeneratedToken::alone(','));
    let mut faults = Vec::new();
    for fault in discipline.faults() {
        let mut expression = Vec::new();
        fault_expr(fault, &mut expression)?;
...
/// One `::macroonz_harness::network::LinkFault::…` expression.
fn fault_expr(fault: &FaultRow, into: &mut Vec<GeneratedToken>) -> Result<(), Overflow> {
    match *fault {
        FaultRow::Drop { at } => positional_fault("DropAt", at, Vec::new(), into),
        FaultRow::Duplicate { at } => positional_fault("DuplicateAt", at, Vec::new(), into),
...
fn positional_fault(
    ...
    absolute_path(&[HARNESS, "network", "LinkFault", arm], into);
...
/// One `::macroonz_harness::network::Tick::at(<n>)` expression.
fn tick_expr(ordinal: u64, into: &mut Vec<GeneratedToken>) -> Result<(), Overflow> {
    absolute_path(&[HARNESS, "network", "Tick", "at"], into);
    into.push(GeneratedToken::group(
        GeneratedDelimiter::Parenthesis,
