# `door` — the three roads a generic attribute walks

A proc host carries tokens and owns no grammar, so the whole road between "an author wrote an attribute" and "a carrier expansion is ready to emit" has to live on this side of the wall.
This home is that road, once per grammar the descriptor home declares.

## What one road does

Each road takes a captured body, reads it through its own grammar, walks the descriptor kind's request to a sealed expansion, composes the carrier's axes out of what that expansion proved, and hands the verified assembly to the carrier's own delivery road.
