...
Its final `bake!` declaration names the two authored enum vocabularies Macroonz must enumerate, the transition rows between them, the explicit absence posture, the projections requested, and any evidence material the existing descriptor adapter must carry.
