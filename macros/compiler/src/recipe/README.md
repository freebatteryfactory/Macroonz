...
The final recipe shell restores the declaration spans of every identifier and exact fragment it re-emits, including an exact effect body and its generated target binding, so compiler diagnostics and editor tooling can trace generated references back to the caller's recipe without moving canonical bytes or identities.
...
Names generated by one standard recipe are accounted within their Rust namespace before rendering.
The recipe-module account owns the generated `baked` module name; the `baked` value namespace owns companion constants and dispatch functions; its type namespace owns relation-table modules, readable-codec refusal types, `TransitionRefusal`, typestate, and direct network or concurrency evidence modules; the crate-root macro namespace owns only the explicitly addressed support carrier.
A repeated name inside one of those declared namespaces refuses at the second declaration that creates the collision.
Imports, glob results, downstream macro output, and names generated by another recipe invocation are outside the recipe input and remain ordinary rustc name-resolution authority rather than inputs to an ambient scan or cross-recipe registry.
