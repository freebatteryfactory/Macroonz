...
Its final `bake!` declaration names the two authored enum vocabularies Macroonz must enumerate, the transition rows between them, the explicit absence posture, the projections requested, and the exported support address required by evidence cargo.
...
Declaration-site companions are assembled inside one generated `baked` child module, and no item is reexported automatically.
Generated-name collisions inside the recipe module refuse before a projector runs.

Test cargo remains inert inside the existing support carrier until an external target invokes the recipe's explicitly named support address.
The support home proves the cargo came from the recipe expansion and keeps the schema gate, parentage, and destination crossing in their existing owners.
...
It does not prove that an effect path resolves, that generated Rust type-checks, that a carrier is invoked, or that a declared transition is correct for any domain.
