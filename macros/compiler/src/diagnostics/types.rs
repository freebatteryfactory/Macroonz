...
use crate::plane::{
    ...
    HumanTextLimit, OwnerFactRef, ProjectionIdentity, RelatedBodySubject, RelatedIssueSubject,
    RepairLimit, ServiceEntrySubject,
};
use crate::token::{SourceCoordinate, SpanHandle, SpanResolutionRefusal};
use core::num::NonZeroUsize;
use macroonz::{Bounded, StopBound};
...
macroonz::closed_register! {
    ...
    pub enum MacrocPhase {
        ...
        /// Planning a projection from informed declaration material.
        Planning = "planning", "planning a projection from informed declaration material";
...
/// with a stand-in: a coordinate written where a table did not reach would read exactly like a coordinate the table resolved, and the reader has no third value to compare it against.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
