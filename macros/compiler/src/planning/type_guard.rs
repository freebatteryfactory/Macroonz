...
use super::{
    ...
    ProjectionKind, ProjectionPlan, SourceDeclarationLimit, UNIVERSAL_QUESTIONS, VerifiedDerived,
};
...
use crate::plane::{
    ...
    MembershipLimit, NonclaimLimit, PlanId, ProjectionIdentity, ProjectionProvenance,
    ProjectionRole, ProjectionTranscript, RenderedRole, encode_length,
};
use crate::question::ExplanationQuestion;
use crate::refusal::{BoundAxis, ProjectionPlanning, ProjectionPlanningIssue};
use core::marker::PhantomData;
use macroonz::{AdmittedLimit, Bounded, ConstLimit, NonEmptyBounded, PositiveLimit};
...
impl<K: ProjectionKind> OwnerContentAccount<K> {
    ...
    ///
    /// Total because the empty dependency set fits every declared magnitude.
    ...
    /// same terms as the other bounded construction roads in this home.
    pub fn captured_over(
        commitment: ProjectionIdentity<CapturedDeclarationSubject>,
        mut dependencies: Vec<ProjectionIdentity<CapturedDeclarationSubject>>,
    ) -> Result<Self, ProjectionPlanning> {
        dependencies.sort_unstable_by(|left, right| left.as_bytes().cmp(right.as_bytes()));
        dependencies.dedup_by(|left, right| left.as_bytes() == right.as_bytes());
    ...
    /// and the commitment at its full thirty-two bytes are [`CauseAnchoring`]'s own canonical spelling.
...
impl<K: ProjectionKind> ProjectionPlan<K> {
    ...
    pub fn planned(
        ...
        let (identity, provenance) = PlanId::derived_with_provenance(ProjectionTranscript::under(
            ProjectionRole::Plan,
            account.anchoring(),
            &claim,
            0,
