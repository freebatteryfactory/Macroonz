...
///
/// The TRIAL axis fills the gate's trials seat, and the EVALUATION axis fills its deferred seat with the generated mutation module.
/// An absent axis renders its seat empty rather than leaving it out, so trials-only, mutation-only, and both use one grammar with one arm.
///
...
pub fn assembled_shell(
    ...
    if stated.addressing != *assembly.addressing() {
        return Err(ShellComposition::NotOneDeclarations(
            CarrierAssembly::established(AssemblyIssue::CarrierRootIsNotTheAssemblys {
                stated: Box::new(assembly.addressing().clone()),
                planned: Box::new(stated.addressing.clone()),
            }),
        ));
    }
    ...
    let deferred = match assembly.evaluation() {
        ...
        AxisCargo::Carried(cargo) => DeferredDelivery::Carried(cargo.proved().cargo()),
    };
    let support = assembly
        .support()
        .map_or(SupportDelivery::Unaddressed, SupportDelivery::Addressed);
