...
use crate::test_descriptor::{DeferredCargo, ShellRendering, SupportMacroName, TrialTablePayload};
...
///
/// **Promotion to proved cargo belongs to the road that owns the source's rendering vocabulary, and the promotion road is crate-internal.**
/// The reading authenticates the complete deferred cargo against the terminal's own proved partition.
/// The deferred cargo itself stays PUBLIC as a declaration value, but holding it claims nothing about any terminal.
///
...
/// One implementation terminal's proved generated mutation module with the optional public support address its mutation helper owns.
#[must_use = "evaluation cargo keeps its proved mutation module and any mutation-owned address inseparable"]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EvaluationCargo {
    proved: ProvedCargo,
...
/// # Authority
///
/// **Every issue here is a fact about COMPOSITION and never about meaning.**
/// What a row says and what the generated mutation module renders were established by their owners.
/// This family establishes that a set of already-proved outputs physically belongs in one exported shell and that the shell is the one this declaration planned.
///
/// # Bounds
///
...
pub enum AssemblyIssue {
    ...
    RootsDisagree {
        ...
        stated: Box<ContentAddressing>,
        /// The root that axis's source terminal stands under.
        carried: Box<ContentAddressing>,
    },
