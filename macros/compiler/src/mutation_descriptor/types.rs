...
/// One owner-declared cause row prepared for mechanical order rendering.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct MutationOrderCause {
    local_key: String,
    spelling: String,
    row: Vec<GeneratedToken>,
}

/// Whether one informed request carries an adjacent-order road.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) enum MutationOrderDeclaration {
    /// This family shape has no adjacent-order mutation road.
    NotApplicable { expression: Vec<GeneratedToken> },
    /// These typed rows are the owner declaration's canonical order.
    Declared(Vec<MutationOrderCause>),
}

/// The complete input the mechanical mutation renderer consumes.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct MutationProjectionRequest {
    declaration: MutationDeclaration,
    point: WallName,
    order_type: Vec<GeneratedToken>,
    production_expression: Vec<GeneratedToken>,
    order_constructor: Vec<GeneratedToken>,
    order: MutationOrderDeclaration,
    alternative_count: u64,
}

/// Why the mechanical mutation renderer did not produce a tree.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(crate) enum MutationRenderRefusal {
    /// The generated tree exceeded the declared token magnitude.
    Unbounded,
}
