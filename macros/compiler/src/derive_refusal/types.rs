...
use crate::diagnostics::{MacrocDiagnostic, ObservedClassification, SiteCoordinate};
...
use crate::planning::{ProjectionDisposition, RefusalFamilyImplementationProjection};
...
use crate::token::SourceCoordinate;
use crate::token::{SpanHandle, SpanTable, TextCapture};
use macroonz::{
    Bounded, CauseId, CauseOrderDeclaration, CompletionPosture, ConstLimit, DeclaredCause,
    DeclaredCauseOrder, FamilyShape, LocalCauseKey, RefusalFamily, RefusalFamilyId,
};
...
/// The crate binding a consumer reaches the refusal contracts through by default.
pub const DEFAULT_CRATE_BINDING: &str = "macroonz";
...
/// # Content
///
/// **The rows are what the documentation commitment stands over, and they are exactly what the semantic commitment sets aside.**
/// A captured declaration is named twice: [`RefusalDeriveSurface::identity`] stands over the declaration's tokens with these attributes dropped, and [`RefusalDeriveSurface::documentation_identity`] stands over that name and these rows in order.
/// A prose-only change therefore preserves the semantic identity and moves only the inspectable documentation identity.
///
/// Both are readings of one captured surface, and the rows are cut from the same token material from which the semantic commitment was derived.
///
/// # Nonclaims
///
/// It states only what was written and where, without deciding what the text means.
...
///
/// # Two commitments, one surface
///
/// One captured surface carries two authored readings.
/// The semantic commitment covers the declaration with documentation attributes set aside, while the documentation commitment covers that semantic name and the ordered captured rows.
/// Implementation, test, codec, and other semantic projections account over the semantic commitment; the documentation commitment remains inspectable captured evidence and is not a projection account in the current roster.
///
...
macro_rules! capture_causes {
    ($(
        ...
        ///
        /// The canonical order below is the declared cause order, not the execution schedule.
...
///
/// A closed sum rather than a bounded collection, because the declaration decides which of three exact role sets applies.
/// No answer is empty: a derivation that would generate nothing is a disposition, not a derivation.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[expect(
    clippy::enum_variant_names,
    reason = "the shared Family prefix is part of this public role-set contract and keeps each imported variant self-describing"
)]
pub enum DerivedMembership {
    ...
    FamilyAndCauseOrder,
    /// The family implementation, the cause-order implementation, and one generated mutation module.
...
///
/// A caller cannot hold this without the plan, the rendering, the proved closure, and the complete explanation all having been produced, having agreed, and having been bound: `RefusalFamilyExpansion::bound` is crate-internal and builds through [`ClosedExpansion::bound`], which refuses a closure proved against another plan and an explanation answered over another plan or another closure.
/// There is no constructor that skips a step and no other public value in this home that carries a token tree.
///
...
pub struct RefusalFamilyExpansion {
    surface: RefusalDeriveSurface,
    expansion: ClosedExpansion<RefusalFamilyImplementationProjection>,
...
pub enum RenderRefusal {
    ...
    Unbounded,
    /// The mutation role was reached without a declared mutation request.
    MutationRequestAbsent,
    /// The captured family identity could not become the mutation point's namespaced coordinate.
    MutationPointNotInformed(ShellDeclarationRefusal),
    /// The adjacent-alternative roster cannot be represented in generated Rust.
    MutationAlternativesUnbounded {
        /// The candidate count that could not be represented.
        observed: usize,
    },
    /// The mechanical mutation renderer refused the informed request.
    MutationRenderingUnbounded,
}
