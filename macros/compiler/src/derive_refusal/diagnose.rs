...
//! # Nonclaims
//!
//! It classifies compiler observations and carries no downstream product cause posture.
//!
...
use crate::diagnostics::{
    ...
    RelatedSetCompletion, RepairAction, ReproductionRoute, SiteCoordinate,
};
...
use crate::token::{CoordinateRole, SourceCoordinate, SpanHandle, SpanTable};
use macroonz::{Bounded, BoundedConstruction, CompletionPosture};
...
const fn planning_observed(issue: &ProjectionPlanningIssue) -> ObservedClassification {
    match issue {
        ProjectionPlanningIssue::MembershipIncomplete { .. } => ObservedClassification::SeatAbsent,
        ProjectionPlanningIssue::ContradictoryOwnerFacts { .. }
        | ProjectionPlanningIssue::UnknownProjectionKind { .. } => {
...
pub fn render_refused<R: RenderedRole>(refusal: RenderRefusal, role: R) -> MacrocDiagnostic {
    match refusal {
        ...
        RenderRefusal::MutationRequestAbsent => mutation_request_refused(
            "the planned mutation role has no informed request",
            vec![0],
            RefusalDeriveFact::APlanStatesItsCompleteOutputSetOrRefuses,
            role,
        ),
        RenderRefusal::MutationPointNotInformed(cause) => mutation_request_refused(
            cause.described(),
            vec![1, cause.slot()],
            RefusalDeriveFact::CauseIdentityIsFamilyAndKey,
            role,
        ),
        RenderRefusal::MutationAlternativesUnbounded { .. }
        | RenderRefusal::MutationRenderingUnbounded => {
            bounded_rendering(RenderedMagnitude::GeneratedTokens, role)
        }
    }
}

fn mutation_request_refused<R: RenderedRole>(
    first: &str,
    mut material: Vec<u8>,
...
const fn mutation_observed(cause: MutationDeclarationCause) -> ObservedClassification {
    match cause {
        ...
        | MutationDeclarationCause::SupportNotDeclared => ObservedClassification::SeatAbsent,
