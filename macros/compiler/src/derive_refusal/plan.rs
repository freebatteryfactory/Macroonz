...
use crate::planning::{
    DigestContract, OwnerContentAccount, PlanDecisions, PlannedMember, PlannedMembership,
    PlannedOutput, ProjectionContext, ProjectionDisposition, ProjectionPlan,
    RefusalFamilyImplementationContent, RefusalFamilyImplementationProjection,
...
#[must_use]
pub fn refusal_family_implementation_kind() -> ProjectionIdentity<ProjectionKindSubject> {
    ProjectionIdentity::derived(ProjectionTranscript::rooted(
        ProjectionRole::DeclaredName,
        b"macroc.kind.refusal-family-implementation-projection",
        0,
    ))
}
...
pub fn content_account(
    draft: &RefusalDerivationDraft,
) -> OwnerContentAccount<RefusalFamilyImplementationProjection> {
...
/// than the account.
#[must_use]
pub fn expansion_context() -> ProjectionContext {
    ProjectionContext {
...
///
/// The output firewall covers every production implementation and the generated mutation module where the helper declared one.
///
/// # Totality
///
/// [`DerivedMembership`] has exactly three answers and each names a statically known set of roles, so there is no count to read, nothing to admit, and no failure to repair.
/// The match below is the whole function: family alone, family with cause order, or all three roles including the generated mutation module.
/// [`PlannedMembership::complete`] settles each magnitude at compile time.
/// Every role in each array is written literally and no two of them are the same seat, which is what the total constructor asks of a caller that names roles rather than reading a count.
#[must_use]
pub fn membership(draft: &RefusalDerivationDraft) -> PlannedMembership<RenderedImplementation> {
    ...
    match draft.declared_membership() {
        ...
        DerivedMembership::FamilyAndCauseOrder => {
            PlannedMembership::complete(member(family), [member(cause_order)])
        }
        DerivedMembership::FamilyCauseOrderAndMutationEvaluation => {
            PlannedMembership::complete(member(family), [member(cause_order), member(mutation)])
        }
    }
}
...
pub fn planned(
    ...
    // The complete set the declaration contributes — the family member, this
    // optional cause-order member, and the optional generated mutation module —
    // is the plan's own membership above. This seat answers the narrower question
    // the explanation protocol asks: what happened to the cause-order projection.
