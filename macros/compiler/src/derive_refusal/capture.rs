...
//! /// <documentation>
//! #[refusal(
//!     crate = <binding>,                     // optional; defaults to `macroonz`
...
//!   roster of shapes.
//! - `order(...)` states the canonical cause order, required exactly when the shape is `single_cause` and admitted only then.
//!   This declared order need not match the order in which the variants are written.
...
//! - The **documentation commitment** stands over the semantic commitment and the ordered documentation rows, and it moves when the prose moves while remaining an inspectable reading of the same captured declaration.
//!
...
use super::types::{
    ...
    SurfaceCaptureRefusal, TrialDeclarationPosture,
};
use crate::mutation_descriptor::{
    MUTATION_ATTRIBUTE, MutationDeclarationCause, MutationDeclarationRefusal, captured_mutations,
};
use crate::plane::{
...
use macroonz::{AdmittedLimit, Bounded, ConstLimit, FamilyShape};
...
pub fn captured(input: &CapturedInput) -> Result<RefusalDeriveSurface, SurfaceCaptureRefusal> {
    ...
    let (mutations, membership) =
        read_mutations(&trees, semantic, input.issued(), attribute.shape)?;
...
fn validate_support_authorship(
    ...
    if let TrialDeclarationPosture::Declared(trials) = trials
        && trials.payload().module().spelling() == declaration.module().spelling()
    {
    ...
    match (trials, declaration.support()) {
        (TrialDeclarationPosture::Declared(_), Some(_)) => {
            Err(MutationDeclarationRefusal::Grammar {
                ...
                at: declaration
                    .support_site()
                    .unwrap_or_else(|| declaration.site()),
            }
            .into())
        }
        (TrialDeclarationPosture::NotDeclared, None) => Err(MutationDeclarationRefusal::Grammar {
...
fn read_mutations(
    ...
    issued: u32,
    shape: FamilyShape,
) -> Result<(MutationDeclarationPosture, DerivedMembership), SurfaceCaptureRefusal> {
    ...
    let Some((body, token)) = found else {
        ...
        return Ok((MutationDeclarationPosture::NotDeclared, membership));
    };
    ...
    if shape != FamilyShape::SingleCause {
        return Err(MutationDeclarationRefusal::Grammar {
    ...
    Ok((
        MutationDeclarationPosture::Declared(Box::new(DeclaredMutations::read(
            commitment,
            declaration,
        ))),
...
///
/// Total: a declaration that wrote no prose carries an empty roster, which is a stated fact and derives a name of its own.
/// It is not the semantic commitment's name and it is not an absence, so two declarations that documented nothing agree in this inspectable reading.
