...
The permitted profile removes only the canonical Instant entry and grants no other ambient-input permission, Rust unsafe permission or authority for measurement readings to enter identities or semantic judgments.
The check boundary is package/feature scoped; it is not a compiler-recognized permission block, and the native operation's semantic limits require owner review and independent behavior controls.
...
- **The tracked tree is not an evidence warehouse:** qualification material graduates to the narrowest owning executable regression, canonical fixture, or public example. A tracked receipt is exceptional and records only an externally observed fact that no executable owner can retain. Campaign chronology, intermediate acceptance state, scratch-subject source, raw output, and reproducibility laboratories remain disposable, while Git history owns how a retained fact was reached.
- **Payloads require an irreducible subject:** a payload beside a receipt is lawful only when the enduring observation cannot be reconstructed from the owning tracked source plus the receipt. Preserving a historical scratch workspace, package archive, or convenient rerun laboratory is not sufficient.
...
- **Cleaning must not erase required inputs:** product implementations, test fixtures and regression controls belong in their tracked owners; an explicitly supplied external comparison input remains outside build output and has no product authority.
No required build or test depends on a previous qualification directory, and qualification subjects are recreated from declared inputs.
