...
//!
//! The old corpus stated that the freshness, proof-disposition, and
//! completeness axes were owned by this chapter; the rebuild moved the AXIS
//! SHAPES to the root calculus (generic, non-erasable parameters) while this
//! home keeps the machinery and the semantics of "complete over what". The
//! old warning survives the move: a flattened anything-freshness enum is the
...
/// The verification lanes — passing a faster lane does not satisfy a
/// requirement assigned to a broader one; every required lane stays visible
/// in the expected denominator. The CI adapter's four lane roles are
/// projections of this register, never a second vocabulary.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Lane {
    ...
    /// Release.
    Release,
    /// Runtime (post-release observation).
    Runtime,
}
...
/// The fourteen denominator comparison axes — expected versus executed,
/// compared across every axis.
pub const DENOMINATOR_AXES: [&str; 14] = [
    "package",
    "target",
    "test-family",
