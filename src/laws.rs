...
mod root {
    ...
    crate::closed_register! {
        /// A roster of exactly sixty-four rows: the stamp's declared supply of
        /// positions, spent to the last one.
        ///
        ...
        /// sentence written sixty-four times to satisfy a habit is a sentence
        /// nobody wrote.
        enum CeilingRoster {
            Row00 = "row-00", "row 00";
            Row01 = "row-01", "row 01";
    ...
    /// law: root.a-stamped-roster-declares-its-own-ceiling — the stamp admits a
    /// sixty-four-row declaration and answers an exact position for every row of
    /// it. The positions come from the stamp's own declared supply rather than
    /// from a running total, so there is no arithmetic in the expansion to
    /// saturate: the first and last rows pin both ends of the supply, and every
    /// row between them answers its own place in `ALL`.
    /// Executed reversal (red twin):
    /// `testpak/tests/compile-fail/a-roster-past-the-stamp-ceiling.rs` adds a
    /// sixty-fifth row and requires the stamp's OWN diagnostic — naming the
    /// ceiling and the supply it is the length of — rather than the compiler's
    /// recursion limit. Without it this law would say only that some unstated
    /// number of rows compiles.
    /// The claim's ceiling: sixty-four is this stamp implementation's authoring
    /// profile and nothing else. This law says the ceiling stands exactly where
    /// the stamp says it stands; it says nothing about whether any vocabulary
    /// should approach it, and the same boundary read through the lifecycle
    /// facade arrives with the lifecycle specimen.
    #[test]
    fn a_stamped_roster_declares_its_own_ceiling() {
        assert_eq!(CeilingRoster::ALL.len(), 64);
        assert_eq!(CeilingRoster::Row00.slot(), 0);
        assert_eq!(CeilingRoster::Row63.slot(), 63);
        assert!(
            CeilingRoster::ALL
                .iter()
                .enumerate()
        ...
        assert_eq!(CeilingRoster::Row63.stable_name(), "row-63");
        assert_eq!(CeilingRoster::Row63.described(), "row 63");
    }
}
