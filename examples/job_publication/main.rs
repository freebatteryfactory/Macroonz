//! Publish a shared definition and its two sites through the existing native command owner.
...
fn main() -> Result<(), String> {
    let (command, values) = configuration::read()?;
    let output = match bake(command, || generate::publication(values)) {
        Ok(output) => output,
        Err(error) => {
